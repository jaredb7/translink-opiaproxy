...
namespace OPIA.API.Contracts.OPIAEntities.Request.Travel
{
    ...
    public class PlanUrlRequest : PlanRequest, IRequest
    {
        ...
        public override string ToString()
        {
            ...
            sb.AppendFormat("&vehicleTypes={0}", String.Join(",", Enumerable.ToArray<int>(VehicleTypes.Select(v => (int) v))));
            sb.AppendFormat("&walkSpeed={0}", (int)WalkingSpeedType);
            sb.AppendFormat("&maximumWalkingDistanceM={0}", MaximumWalkingDistanceM);
            sb.AppendFormat("&serviceTypes={0}", String.Join(",", Enumerable.ToArray<int>(ServiceTypes.Select(s => (int) s))));
            sb.AppendFormat("&fareTypes={0}", String.Join(",", Enumerable.ToArray<int>(FareTypes.Select(f => (int) f))));
